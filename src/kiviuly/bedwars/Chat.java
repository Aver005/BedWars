...
public class Chat implements Serializable
{
	...
	private ArrayList<Chat> messageInheritance = new ArrayList<>();
	...
	{
		if (!startSymbol.isEmpty() && !msg.startsWith(startSymbol)) {return;}
		if (!startSymbol.isEmpty() && msg.startsWith(startSymbol)) {msg = msg.substring(1);}
		...
		for(UUID id : players)
		{
			...
			p.sendMessage(prefix + pl.getName() + suffix + msg);
		}
		
		for(Chat c : messageInheritance)
		{
			if (c == null) {continue;}
			c.SendMessage(msg, pl);
		}
	...
	public void addMessageInheritance(Chat gameChat) 
	{
		if (messageInheritance.contains(gameChat)) {return;}
		messageInheritance.add(gameChat);
	}

	public ArrayList<Chat> getMessageInheritanceChats() {
		return messageInheritance;
	}

	public void setMessageInheritanceChats(ArrayList<Chat> messageInheritance) {
		this.messageInheritance = messageInheritance;
	}

	public void MessageStartedSymbol(String startSymbol) 
	{
		this.startSymbol = startSymbol;
	}

	public String getMessageStartedSymbol() {
		return startSymbol;
	}

	public boolean hasStartedSymbol() 
	{
		return !startSymbol.isEmpty();
	}
}
